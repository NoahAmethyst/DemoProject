...
public final class RedisUtil {
    ...
    public Object get(String key) {
        if (key == null) {
            throw new RuntimeException("key必须不为null");
        }
     ...
     * @return
     */
    public boolean limitedLSet(String key, Object value, Integer length) {
        return limitedLSet(key, value, length, 0);
    }


    public boolean limitedLSet(String key, Object value, Integer length, long time) {
        try {
            int thisSize = Math.toIntExact(lGetListSize(key));
            if (length <= thisSize) {
                int overSize = thisSize - length + 1;
                redisTemplate.opsForList().trim(key, overSize, -1);
            }
            redisTemplate.opsForList().rightPush(key, value);
            if (time > 0)
                expire(key, time);
     ...
     * @return 移除的个数
     */
    public <T> T execute(String script, List<String> keys, List<String> params, Class returnType) {
        try {
            RedisScript<T> redisScript = new DefaultRedisScript(script, returnType);
            return redisTemplate.execute(redisScript, keys, params.toArray(new String[params.size()]));
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
